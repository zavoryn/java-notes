...
> **中文总结**: Cognition（Devin 开发商）的反面观点文章，主张多数场景下不应构建多 Agent 系统。主要论点：上下文共享困难（"电话游戏"问题）、协调成本高、调试困难、单体 Agent 通常更可靠。
...
That said, if you're new to agent-building, there are lots of resources on how to set up the basic scaffolding. But when it comes to building serious production applications, it's a different story.
